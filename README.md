...
6. **配 obsidian-git 自动同步**：待插件装好后，把「📎 附录」里那份 `obsidian-git/data.json` **整段写入** `.obsidian/plugins/obsidian-git/data.json`（该 JSON 已在 obsidian-git 2.38.x 实测可用）。⚠️ 写入时若 Obsidian 开着，写完让用户执行 `Ctrl+P → Reload app without saving` 重载以加载新设置。
7. **配 Templater 模板目录**：把附录里的 `templater-obsidian/data.json` 写入对应路径，或提示用户在 设置 → Templater → Template folder location 填 `_templates`（个人插件配置，不随仓库下发）。
...
**省事做法（推荐）**：关掉 Obsidian，把「📎 附录」里那份 `obsidian-git/data.json` 整段保存到
`<仓库>\.obsidian\plugins\obsidian-git\data.json`，再开 Obsidian 即可（已实测可用，免去逐项点）。
...
### 4. 配置 Templater 模板目录
设置 → Templater → Template folder location → 填 `_templates`（或粘贴附录的 `templater-obsidian/data.json`）。
...
> 这两份文件属**个人插件配置**，按 `.gitignore` 不随仓库下发，所以每位成员需在本机放一次。
> 放置前提：对应插件已安装（插件目录已存在）。若 Obsidian 正开着，放完执行 `Ctrl+P → Reload app without saving` 重载生效。

**`.obsidian/plugins/obsidian-git/data.json`**（已在 obsidian-git 2.38.x 实测可用）：
```json
{
...
**`.obsidian/plugins/templater-obsidian/data.json`**（关键是 `templates_folder` 指向 `_templates`）：
```json
{
